...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
        ...
        List<String> syllable = this.getTokens("[aeiouAEIOU]+", word);
        int zero_count = 0;
        if (syllable.size() == 0) {
            ++zero_count;
        }
        return syllable.size() + flag + zero_count;
	}
	...
	public double getFleschScore()
	{
	    // TODO: Implement this method
	     double flesch_Score = 206.835 - 1.015 * ((double)this.getNumWords() * 1.0 / (1.0 * (double)this.getNumSentences())) - 84.6 * (1.0 * (double)this.getNumSyllables() / (1.0 * (double)this.getNumWords()));
        return flesch_Score;
	}
