...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
        ...
        if (tokens.size() != 0)
        {
            ...
            {
                if (this.isWord(each_token)) {
                    ++this.numWords;
                    this.numSyllables += this.countSyllables(each_token);
                    continue;
                }
                ++count_sen;
            }
            if (count_sen == 0) {
                this.numSentences = 1;
            } else if (count_sen > 0 && this.check_last_char()) {
                this.numSentences = count_sen;
            } else if (count_sen > 0 && !this.check_last_char()) {
                this.numSentences = count_sen + 1;
            }
        }
        else
        {
	 ...
	 public boolean check_last_char() {
        char last_char;
        if (this.getText().length() != 0 && ((last_char = this.getText().charAt(this.getText().length() - 1)) == '?' || last_char == '.' || last_char == '!')) {
            return true;
        }
        return false;
    }
