...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for (int index = 0; index <= s.length(); ++index) {
            for (int charCode = 97; charCode <= 122; ++charCode) {
                StringBuffer sb = new StringBuffer(s);
                if (currentList.contains((sb = sb.insert(index, (char)charCode)).toString()) || wordsOnly && !this.dict.isWord(sb.toString()) || s.equals(sb.toString())) continue;
                currentList.add(sb.toString());
            }
        }
	}
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for (int index = 0; index < s.length(); ++index) {
            StringBuffer sb = new StringBuffer(s);
            if (currentList.contains((sb = sb.deleteCharAt(index)).toString()) || wordsOnly && !this.dict.isWord(sb.toString()) || s.equals(sb.toString())) continue;
            currentList.add(sb.toString());
        }
	}
	...
	public List<String> suggestions(String word, int numSuggestions) {
        ...
        for (int k = 1000; !queue.isEmpty() && numSuggestions > 0 && k > 0; --k) {
            curr = (String)queue.remove(0);
            tempList = this.distanceOne(curr, true);
            for (Object n : tempList) {
                if (visited.contains(n)) continue;
                visited.add(n.toString());
                queue.add(n.toString());
                if (!this.dict.isWord(n.toString())) continue;
                retList.add(n.toString());
                --numSuggestions;
            }
        }
        return retList;
