...
class TrieNode {
    ...
    public ArrayList<TrieNode> getAllChild() {
        Set<Character> child = this.children.keySet();
        ArrayList<TrieNode> allchild = new ArrayList<TrieNode>();
        Iterator<Character> iterator = child.iterator();
        while (iterator.hasNext()) {
            char c = iterator.next().charValue();
            allchild.add(this.getChild(Character.valueOf(c)));
        }
        return allchild;
    }
}
